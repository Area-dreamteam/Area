"""OAuth state management for mobile authentication flows.
...
when cookies are not available (e.g., mobile external browser flows).
"""
...
def store_oauth_state(state: str, user_id: int) -> None:
    """Store OAuth state token with associated user ID.
    ...
    with _state_lock:
        ...
        logger.debug(f"Stored state: {state} -> user_id={user_id}, expires_in={STATE_EXPIRY_SECONDS}s, total_states={len(_oauth_states)}")
...
def get_user_from_state(state: str) -> Optional[int]:
    ...
    """
    with _state_lock:
        logger.debug(f"Looking up state: {state}, available_states={list(_oauth_states.keys())}")
...
def cleanup_expired_states() -> None:
    ...
    with _state_lock:
        expired = [
            state for state, (_, expiry) in _oauth_states.items()
