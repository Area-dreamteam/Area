    ...
    class create_event(Reaction):
        ...
        def execute(self, session, area_reaction, user_id):
            try:
                ...
                logger.info(
                    f"{self.service.name} - {self.name} - Created event '{summary}' in calendar '{calendar_name}' - User: {user_id}"
                )
            except GoogleCalendarApiError as e:
                logger.error(f"{self.service.name}: {e}")
        ...
        def execute(self, session, area_reaction, user_id):
            try:
                ...
                if r.status_code != 200:
                    raise GoogleCalendarApiError(f"Failed to create event: {r.text}")
                logger.info(
                    f"{self.service.name} - {self.name} - Created detailed event '{summary}' in calendar '{calendar_name}' - User: {user_id}"
                )
            except GoogleCalendarApiError as e:
                logger.error(f"{self.service.name}: {e}")
