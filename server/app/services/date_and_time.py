...
class DateAndTime(Service):
    class every_hour_at(Action):
        def __init__(self) -> None:
            ...
            super().__init__("Déclenché une fois par heure à :00, :15, :30 ou :45 minutes après l'heure.", config_schema)

        def check(
            self, session: Session, area_action: AreaAction, user_id: int
        ) -> bool:
            current_time = datetime.now()
            selected_minute: int = int(get_component(area_action.config, "select_interval", "values"))
