...
class RiotDevApi(AreaApi):
    ...
    def _get_last_match_info_complete(
        self, api_key: str, game_name: str, tag_line: str
    ):
...
class RiotDev(Service):
    ...
    class win_game(Action):
        ...
        def check(
            ...
            player_game_name = get_component(
                area_action.config, "player_game_name", "values"
            )
            player_tag_line = get_component(
                area_action.config, "player_tag_line", "values"
            )
            last_state = area_action.last_state

            match_info = riot_dev_api.get_last_match_info(
                dev_api_key, player_game_name, player_tag_line
            )
            if match_info is None:
                return False
