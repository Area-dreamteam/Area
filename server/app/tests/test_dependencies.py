...
class TestGetCurrentUserNoFail:
    ...
    def test_get_current_user_no_fail_invalid_user_id(self):
        ...
        with patch('dependencies.auth.decode_jwt', return_value={"sub": "not_a_number"}):
            result = get_current_user_no_fail(mock_session, token)
    ...
    def test_get_current_user_no_fail_bearer_handling(self):
        ...
        with patch('dependencies.auth.decode_jwt', return_value={"sub": "1"}) as mock_decode:
            result = get_current_user_no_fail(mock_session, token_with_bearer)
            assert result == mock_user
            mock_decode.assert_called_with("jwt_token")  # Should strip "Bearer "
    ...
    def test_get_current_user_no_fail_type_error_handling(self):
        ...
        # Mock a TypeError when converting user_id to int
        with patch('dependencies.auth.decode_jwt', return_value={"sub": "1"}):
    ...
    def test_get_current_user_no_fail_value_error_handling(self):
        ...
        # Mock a ValueError when converting user_id to int
...
class TestRoleDependencies:
    ...
    def test_check_admin_function_not_admin(self):
        ...
        mock_user = Mock()
        mock_user.role = Role.USER  # Not admin
    ...
    def test_current_user_and_admin_are_annotated_types(self):
        ...
        # These are Annotated types, not callable functions
...
class TestSessionDependency:
    ...
    def test_session_dep_type(self):
        ...
        # SessionDep should be importable and usable as a dependency
        assert SessionDep is not None
        
        # It should be a type annotation for dependency injection
        # The actual implementation depends on FastAPI's dependency system
...
class TestDependencyEdgeCases:
    ...
    def test_get_current_user_with_malformed_bearer(self):
        ...
        mock_session = Mock(spec=Session)
        token = "Bearertoken"  # Missing space
    ...
    def test_get_current_user_no_fail_database_error(self):
        ...
        with patch('dependencies.auth.decode_jwt', return_value={"sub": "1"}):
            # get_current_user_no_fail may or may not handle database errors
            try:
                result = get_current_user_no_fail(mock_session, token)
                # If it returns None, that's graceful handling
                assert result is None
            except Exception:
    ...
    def test_get_current_user_multiple_bearer_prefixes(self):
        ...
        with patch('dependencies.auth.decode_jwt', return_value=None) as mock_decode:
            ...
            # Should try to decode "Bearer actual_token" (after stripping first "Bearer ")
...
class TestDependencyIntegration:
    ...
    def test_authentication_flow_integration(self, mock_decode_jwt):
        ...
        # Test regular user dependency
        user_result = get_current_user(mock_session, token)
        assert user_result == mock_user
        
        # Note: CurrentAdmin is an Annotated type, not a function
        # Integration with FastAPI dependency injection is tested at API level
        
        # Test no-fail dependency
    ...
    def test_permission_escalation_prevention(self, mock_decode_jwt):
        ...
        # Regular user dependency should work
        user_result = get_current_user(mock_session, token)
        assert user_result == mock_user
        
        # Note: Admin checks happen in check_admin function, not CurrentAdmin directly
        # CurrentAdmin is an Annotated type for FastAPI dependency injection
    
    def test_dependency_error_consistency(self):
        """Test that dependencies return consistent error types"""
        mock_session = Mock(spec=Session)
        
        # All authentication failures should raise HTTPException with 403
        test_cases = [
            (None, "Token missing."),
            ("", "Token missing."),
        ...
        for token, expected_detail in test_cases:
            with patch('dependencies.auth.decode_jwt', return_value=None):
                ...
                if token is None or token == "":
                    ...
                    assert "Invalid" in exc_info.value.detail or "Token" in exc_info.value.detail