...
class TodoistApi:
    ...
    def get_projects_id(self, token, project_name):
        return next((project.project_id for project in todoist_api.get_projects(token) if project.name == project_name), None)
    ...
    def get_tasks(self, token, project_name=None) -> list[Task]:
        ...
        r = requests.get(f"{base_url}?{urlencode(params)}", headers={"Authorization": f"Bearer {token}"})
    ...
    def create_task(self, token, content, project_name=None):
        ...
        r = requests.post(f"{base_url}", data=params, headers={"Authorization": f"Bearer {token}"})
