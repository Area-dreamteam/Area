...
function FilterApplets(text: string, applets: PrivateApplet[] | null) {
  if (applets === null) return applets
  if (text === 'Enabled') {
    const filteredApplets = applets.filter((applet) => applet.enable)
...
export default function My_applet() {
  const [privateApplets, setPrivateApplets] = useState<PrivateApplet[] | null>(
    null
  )
  const [publicApplets, setPublicApplets] = useState<PrivateApplet[] | null>(
    null
  )
    ...
    <div className="w-screen">
      ...
      <Applets
        ...
        applets={
          page == 'Published'
            ? publicApplets
            : FilterApplets(page, privateApplets)
        }
        className="mt-[50px] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 justify-items-center"
        boxClassName="rounded-xl w-[250px] h-[300px] hover:cursor-pointer mb-[20px] border-black border-[2px]"
        onClick={
          page == 'Published'
            ? redirectToUnpublishableApplet
