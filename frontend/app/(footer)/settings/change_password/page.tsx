...
export default function changePassword()
{
    ...
    return (
        <div className="mx-auto mt-[40px] w-[75%] font-bold">
            ...
            {error &&
                Warning("Invalid passwords", "New password and confirmation aren't the same")
            }
            <button className="rounded-button centered inverted px-[5%] py-[3%] mt-[5%] disabled:bg-gray-400 disabled:cursor-auto" disabled={newPassword.length < 8 || confirmNewPassword.length < 8 || newPassword.length != confirmNewPassword.length} onClick={() => {
            if (!sendForm(currentPassword, newPassword, confirmNewPassword, setError)) {
                console.log(error);
            }}}>
                Change
            </button>
