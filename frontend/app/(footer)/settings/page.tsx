...
function profileLabels(text: string) {
  return <h2 className="text-[30px] mt-[40px] font-bold">{text}</h2>
}
...
function oauthLink(
  ...
  return (
    <p
      className="special-link simple-text hover:cursor-pointer"
      style={{ color: linkColor }}
      onClick={() =>
        service.connected
          ? disconnectOauth(service.id, setUpdate)
          : connectOauth(service.name, setUpdate)
      }
    >
...
function DeleteAccount() {
  return (
    <Link
      href={'/'}
      className="mb-[30px] text-center text-[18px]"
      style={{ color: '#ff0000' }}
      onClick={fetchDeleteMyself}
    >
      Delete my account
    </Link>
  )
...
function LinkedAccounts(
  ...
  return <div className="w-[75%] block mx-auto mb-5 mt-5">{linked}</div>
}
...
function Profile({ profile }: PersonnalInfoProp) {
  ...
  return (
    <form onSubmit={sendForm} className="mx-auto mt-[40px] w-[75%] font-bold">
      <h1 className="title mt-[40px] font-bold">Account Settings</h1>
      <hr />
