...
function ActionButton({
  ...
  return (
    <div
      className="mx-auto mt-[10%] w-[75%] h-[100px] md:h-[170px] rounded-xl text-white flex items-center"
      onClick={() => ''}
      style={{ background: disable ? 'grey' : 'black' }}
    >
      <h1 className="flex-1 title inverted">
        {buttonText}
        {chosen ? (
          <p className="ml-[20px] tiny-text inverted">
            {chosen.name.replaceAll('_', ' ')}
          </p>
        ) : (
          replacementText
        )}
      ...
      {!disable && !chosen && (
        <Button
          className="mr-[20px] rounded-full text-black hover:bg-white bg-white hover:cursor-pointer px-[30px] py-[20px] font-bold w-[100px] text-[20px]"
      ...
      {chosen && (
        <div>
          <button
            className="mr-[20px] mb-[5%] py-[5%] rounded-button w-[75%] font-bold"
            onClick={() => setIsChoosing(true)}
          >
            Edit
          </button>
          <button
            className="mr-[20px] py-[5%] rounded-button w-[75%] font-bold"
...
function LeftUpButton({ text, act, param, color = 'black' }: UpButtonProp) {
  return (
    <Button
      className={`ml-[10%] mt-[20%] rounded-full border-${color} text-${color} hover:bg-transparent bg-transparent border-[4px] hover:cursor-pointer px-[30px] py-[20px] font-bold w-[120px] text-[20px]`}
      onClick={() => act(param)}
    >
      {text}
    </Button>
...
function Creation({
  ...
  const [title, setTitle] = useState<string>(
    `if ${action?.name}, then ${reaction?.name}`
  )
      ...
      {validating ? (
        <div>
          <div className="rounded-b-xl bg-black text-white font-bold w-screen h-[450px]">
            <div className="grid grid-cols-4">
              <LeftUpButton
                text="Back"
                act={(param: boolean | string) =>
                  setValidating(param as boolean)
                }
                param={false}
                color="white"
              />
            ...
            <Input
              className="block mx-auto w-[75%] h-[10%] bg-white text-black"
              defaultValue={title}
          ...
          <div className="centered mt-[30px]">
            <button
              className="rounded-button inverted px-[5%] py-[3%]"
              onClick={() =>
                action &&
                reaction &&
                createApplet(action, reaction, title, actConfig, reactConfig)
              }
              disabled={title === ''}
            >
              Finish
        ...
        <div>
          <div className="grid grid-cols-4">
            <LeftUpButton
              text="Cancel"
              act={(param: boolean | string) => redirect(param as string)}
              param={'/my_applets'}
            />
          ...
          <ActionButton
            ...
            setChosen={setReaction}
            chosen={reaction}
          />
          {action != null && reaction != null && (
            <div>
              <button
                className="mt-[5%] rounded-button inverted block mx-auto"
                onClick={() => setNbReactions(nbReactions + 1)}
              >
                +
              </button>
              <ValidateButton
                arg={true}
                clickAct={setValidating}
                text="Continue"
                addToClass={'mt-[100px] mb-[5%]'}
                inverted={true}
              />
            </div>
          )}
...
function SelectElement({ content, config, handleChange }: SelectElementProp) {
  ...
  return (
    <Select
      onValueChange={(v) => defineChoice(v, setVal, handleChange, config)}
      value={val}
    >
      ...
      <SelectContent className="text-black">
        <SelectGroup>{selectItemsBlock}</SelectGroup>
      </SelectContent>
    </Select>
  )
}
...
function CheckboxElement({ content, config, handleChange }: SelectElementProp) {
  ...
  const [checkboxes, setCheckboxes] =
    useState<{ [key: string]: boolean }[]>(initialBoxes)
  ...
  return (
    <div key={config.name}>
      ...
      {config.type == 'input' && (
        <Input
          onChange={(e) =>
            defineChoice(e.target.value, () => '', handleChange, config)
          }
        />
      )}
...
function allTriggersValid(
  configResp: ConfigRespAct[],
  configReq: ConfigReqAct[] | undefined
) {
  if (typeof configReq === 'undefined') return false
  if (configReq.length == 0) return true
  if (configResp.length == 0) return false
...
function ChooseTrigger({
  ...
  const [trigger, setTrigger] = useState<
    SpecificAction | SpecificReaction | null
  >(null)
    ...
    >
      <div className="grid grid-cols-4 ">
        <LeftUpButton
          text="Back"
          act={(param: boolean | string) =>
            setChoosingTrigger(param as boolean)
          }
          param={false}
          color="white"
        />
        ...
        <div className="flex flex-col mb-[20px] font-bold col-span-4 mx-auto">
          ...
          {trigger ? (
            <DisplayAllTrigger
              config={trigger.config_schema}
              configResp={configResp}
              setConfigResp={setConfig}
            />
          ) : (
  ...
  return (
    <div>
      {choosingTrigger && act ? (
        ...
        <div>
          <div
            className="grid grid-cols-4 text-white w-screen rounded-b-xl"
            style={{ background: service.color }}
          >
            <LeftUpButton
              text="Back"
              act={() => reinitAll({ setService, setAction })}
              param={true}
              color="white"
            />
          ...
          {acts && acts.length > 0 ? (
            <div className=" w-[95%] mx-auto mt-[25px] grid xl:grid-cols-5 lg:grid-cols-4 grid-cols-3 gap-2">
              {acts.map((act) => (
                <div
                  key={act.id}
                  className="rounded-xl md:w-[250px] w-[150px] md:h-[250px] h-[150px] hover:cursor-pointer relative"
                  style={{ backgroundColor: service.color }}
                  onClick={() => selectAct(setChoosingTrigger, setAction, act)}
                >
...
function ChooseService({
  ...
  const [chosenService, setChosenService] = useState<SpecificService | null>(
    null
  )
  ...
  useEffect(() => {
    if (!checkedConnection || !chosenService) return
        ...
        <div>
          <div className="grid grid-cols-4">
            <LeftUpButton
              text="Back"
              act={(param: boolean | string) =>
                setChoosingAction(param as boolean)
              }
              param={false}
            />
          ...
          <Input
            className="w-[400px] mx-auto block mt-[50px] border-[4px] h-[50px] text-[20px] placeholder:text-[20px]"
          ...
          <Services
            search={search}
            services={services}
            className="mt-[50px] grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 w-fit mx-auto gap-[5px]"
            boxClassName="rounded-xl w-[200px] h-[200px] hover:cursor-pointer relative border-black border-[1px]"
            onClick={setSelected}
          />
        </div>
      )}
      {((selected && serviceConnected && chosenService?.oauth_required) ||
        (selected && chosenService && !chosenService.oauth_required)) && (
        <ChooseAct
          act={act}
          service={selected}
    ...
    <div>
      ...
      {choosingAction && (
        <ChooseService
          ...
          setAction={setAction}
          type="actions"
          setConfig={setActConfig}
          choosingAction={choosingAction}
          setChoosingAction={setChoosingAction}
          configResp={actConfig}
        />
      )}
      {choosingReaction && (
        <ChooseService
          act={reaction}
          setAction={setReaction}
          type="reactions"
          choosingAction={choosingReaction}
          setConfig={setReactConfig}
          setChoosingAction={setChoosingReaction}
          configResp={reactConfig}
        />
      )}
    </div>
  )
